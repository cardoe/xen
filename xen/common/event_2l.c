...
static void evtchn_2l_xs62_set_pending(struct vcpu *v, struct evtchn *evtchn)
{
    struct domain *d = v->domain;
    unsigned int port = evtchn->port;
...
static void evtchn_2l_xs62_unmask(struct domain *d, struct evtchn *evtchn)
{
    struct vcpu *v = d->vcpu[evtchn->notify_vcpu_id];
    unsigned int port = evtchn->port;
...
static bool_t evtchn_2l_xs62_is_pending(struct domain *d, evtchn_port_t port)
{
    unsigned int max_ports = BITS_PER_EVTCHN_WORD(d) * BITS_PER_EVTCHN_WORD(d) *
        WORDS_PER_EVTCHN_SEL_BIT_XS62;
...
static bool_t evtchn_2l_xs62_is_masked(struct domain *d, evtchn_port_t port)
{
    unsigned int max_ports = BITS_PER_EVTCHN_WORD(d) * BITS_PER_EVTCHN_WORD(d) *
        WORDS_PER_EVTCHN_SEL_BIT_XS62;
...
static void evtchn_2l_xs62_print_state(struct domain *d,
                                  const struct evtchn *evtchn)
{
    struct vcpu *v = d->vcpu[evtchn->notify_vcpu_id];
...
static const struct evtchn_port_ops evtchn_port_ops_2l_xs62 =
{
    .set_pending   = evtchn_2l_xs62_set_pending,
    .clear_pending = evtchn_2l_clear_pending,
    .unmask        = evtchn_2l_xs62_unmask,
    .is_pending    = evtchn_2l_xs62_is_pending,
    .is_masked     = evtchn_2l_xs62_is_masked,
    .print_state   = evtchn_2l_xs62_print_state,
};
